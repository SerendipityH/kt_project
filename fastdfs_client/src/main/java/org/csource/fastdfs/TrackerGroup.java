  ...
  public TrackerServer getConnection() throws IOException {
    ...
    for (int i = 0; i < this.tracker_servers.length; i++) {
      ...
      try {
        ...
        System.err
            .println("connect to server " + this.tracker_servers[i].getAddress().getHostAddress()
  ...
  public Object clone() {
    ...
    for (int i = 0; i < trackerServers.length; i++) {
      trackerServers[i] = new InetSocketAddress(
          this.tracker_servers[i].getAddress().getHostAddress(), this.tracker_servers[i].getPort());
    }
