...
public class TestAppender1 {
  ...
  public static void main(String args[]) {
    ...
    try {
       ...
       * System.out.println("getStoreStorage fail, error code: " + tracker.getErrorCode()); return;
       * }
       */
      ...
      System.out.println(
          "upload_appender_file1 time used: " + (System.currentTimeMillis() - startTime) + " ms");

      /*
       * group_name = ""; appender_file_id = client.upload_appender_file1(group_name, file_buff,
       * "txt", meta_list);
       */
      if (appender_file_id == null) {
        ...
        if (servers == null) {
          ...
          for (int k = 0; k < servers.length; k++) {
            System.err
                .println((k + 1) + ". " + servers[k].getIpAddr() + ":" + servers[k].getPort());
          }
          System.err.println("");
        }
        ...
        startTime = System.currentTimeMillis();
        errno = client.set_metadata1(appender_file_id, meta_list,
            ProtoCommon.STORAGE_SET_METADATA_FLAG_MERGE);
        System.out
            .println("set_metadata time used: " + (System.currentTimeMillis() - startTime) + " ms");
        if (errno == 0) {
        ...
        System.out
            .println("append_file time used: " + (System.currentTimeMillis() - startTime) + " ms");
        if (errno == 0) {
          System.err.println(client.get_file_info1(appender_file_id));
        } else {
        ...
        System.out
            .println("delete_file time used: " + (System.currentTimeMillis() - startTime) + " ms");
        if (errno == 0) {
      ...
      if (appender_file_id != null) {
        ...
        errno = client.download_file1(appender_file_id, 0, 0,
            "c:\\" + appender_file_id.replaceAll("/", "_"));
        if (errno == 0) {
      ...
      if (appender_file_id != null) {
        ...
        startTime = System.currentTimeMillis();
        errno = client.append_file1(appender_file_id, f.length(),
        ...
        startTime = System.currentTimeMillis();
        errno = client.modify_file1(appender_file_id, 0, f.length(),
            new UploadLocalFileSender(local_filename));
        System.out
            .println("modify_file time used: " + (System.currentTimeMillis() - startTime) + " ms");
        if (errno == 0) {
          System.err.println(client.get_file_info1(appender_file_id));
        } else {
        ...
        System.out.println(
            "truncate_file time used: " + (System.currentTimeMillis() - startTime) + " ms");
        if (errno == 0) {
          System.err.println(client.get_file_info1(appender_file_id));
        } else {
