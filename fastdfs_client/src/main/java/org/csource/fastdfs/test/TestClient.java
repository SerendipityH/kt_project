...
public class TestClient {
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      System.out
          .println("upload_file time used: " + (System.currentTimeMillis() - startTime) + " ms");

      /*
       * group_name = ""; results = client.upload_file(group_name, file_buff, "txt", meta_list);
       */
      if (results == null) {
        ...
        if (results != null) {
          System.err
              .println("slave file group_name: " + results[0] + ", remote_filename: " + results[1]);
      ...
      if (results != null) {
        ...
        file_ext_name = null;
        startTime = System.currentTimeMillis();
        results = client.upload_file(group_name, master_filename, prefix_name, local_filename, null,
            meta_list);
        System.out
            .println("upload_file time used: " + (System.currentTimeMillis() - startTime) + " ms");
        if (results != null) {
      ...
      results = client.upload_file(null, f.length(), new UploadLocalFileSender(local_filename),
          file_ext_name, meta_list);
      if (results != null) {
        group_name = results[0];
        remote_filename = results[1];
